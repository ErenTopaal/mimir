...
# - AVAX_BENCH_DETECT_MD: path to the detect instructions file
# - AVAX_BENCH_CODEX_TIMEOUT_SECONDS: optional max runtime (default 10800)
...
TIMEOUT_SECONDS="${AVAX_BENCH_CODEX_TIMEOUT_SECONDS:-10800}"
if ! [[ "${TIMEOUT_SECONDS}" =~ ^[0-9]+$ ]]; then
  echo "invalid AVAX_BENCH_CODEX_TIMEOUT_SECONDS=${TIMEOUT_SECONDS}" >&2
  exit 2
fi

# Render instructions where Codex will read them.
cp "${AVAX_BENCH_DETECT_MD}" "${AGENT_DIR}/AGENTS.md"
