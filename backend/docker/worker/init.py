...
RUNNER_DIR = Path(os.getenv('AVAX_BENCH_RUNNER_DIR') or '/opt/avaxbench/worker_runner')
...
def _run_codex_detect(*, openai_token: str, key_mode: str) -> Path:
    ...
    env['AVAX_BENCH_DETECT_MD'] = str(DETECT_MD_PATH)
...
async def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix='avaxbench-worker-') as tmpdir:
